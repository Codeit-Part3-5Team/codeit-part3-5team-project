...
#
#    인명 탐지 — 규칙 + NER 하이브리드로 구현 (2026-06-23):
#      정규식만으로는 인명("김철수")과 일반 명사 구분이 어려워(부서명·기관명 조각
#      오탐), 규칙 1차 필터(이름+직위/담당자:이름)로 후보를 좁힌 뒤
#      한국어 NER(KoELECTRA-modu-ner)로 PER 태그를 검증하는 하이브리드 도입.
#      → evaluation/detect_person_names.py
#
#    실측 결과 (본 데이터 100건):
#      탐지 담당자 인명 38명 / 26개 문서. 기관 매칭(classify_person_names.py)
#      결과 전원이 공공기관·공단·공사·국공립대·교육청 직무 담당자로,
#      정보공개법 9조1항6호 라목·마목상 공개대상. 사적 마스킹 대상 0건.
#      → 추정 카운트(키워드 1,689건)를 실측(38명)으로 대체.
#
#    한계:
#      NER도 경계 케이스(서비스명 등) 잔여 가능. 본 데이터 전수가 공공 담당자였으나,
#      사립·민간위탁 등 확장 시 마목 비해당 케이스는 개별 판정 필요
#      (탐지기가 그 케이스를 잡아내는 인프라 역할).
# =====================================================================
import re
import unicodedata
