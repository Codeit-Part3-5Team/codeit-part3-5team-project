...
def test_detector_handles_variants():
    ...
    assert scan_restricted("０１０－１２３４－５６７８").get(
        "mobile"
    ), "전각 숫자 우회 탐지 실패"
    assert scan_restricted("010\u20131234\u20135678").get(
        "mobile"
    ), "특수대시 우회 탐지 실패"
...
def test_landline_preserved(final_chunks):
    ...
    count = sum(
        len(landline_re.findall(ch.get("page_content", ""))) for ch in final_chunks
    )
...
def test_token_not_broken(final_chunks):
    ...
    for ch in final_chunks:
        ...
        for m in re.findall(
            r"\[(전화번호|이메일|휴대폰|사업자등록번호|법인등록번호|계좌번호|주민등록번호)[^\]]*\]",
            pc,
        ):
            full = f"[{m}]"
